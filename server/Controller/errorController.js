...
const handleDuplicateFieldDB = (err) => {
  console.log("duplicate name ",err.keyValue.email)
  const message = `this email : ${err.keyValue.email} is allready exists! Please use another email`;
  return new AppError(message, 400);
}
const handleValidationErrorDB = (err) =>{
  console.log("in the handle validate validation error ",err)
